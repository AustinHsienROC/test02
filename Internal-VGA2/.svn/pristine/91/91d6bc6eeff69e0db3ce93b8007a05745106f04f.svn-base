    ...
    for( row = 1 ; row < (imgDimensionY-1); ++row)
    {
        ...
        for(col=1; col<(imgDimensionX-1); ++col)
        {
            ...
            *(Sobel_Magnitude + row*imgDimensionX + col) = SAT8 (abs(sobel_H) + abs(sobel_V));
            //* TODO: 請勿用atan
            *(Sobel_Theta     + row*imgDimensionX + col) = (CHAR)(atan((float)sobel_H/(float)sobel_V)* 180 / PI); // 這是因為Verical/Horizontal 寫反了故H/V顛倒
...
/**
* @brief	  1)將src copy row = 0 ,end ; col = 0 ,end copy to dst 
              2)dst = 0 if src 四周全0 , dst = 255 if src 四周有值
...
void Filter2D::CannyEdge( UCHAR *Canny_img , UCHAR *src, INT32 imgDimensionX, INT32 imgDimensionY )
{
    ...
    // Compensation of edge region of image(第一行第一列塗白)
